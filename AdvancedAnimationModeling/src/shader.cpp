...
shader * shader::CreateShaderProgram(const std::string & vertShader, const std::string & fragShader)
{
    ...
    if (!shaderProgram->CompileShaderFromFile(vertShader.c_str(), shader_type::VERTEX))
    {
        std::cout << vertShader << std::endl << shaderProgram->Log() << std::endl;
    }
    if (!shaderProgram->CompileShaderFromFile(fragShader.c_str(), shader_type::FRAGMENT))
    {
...
bool shader::CompileShaderFromFile(const char * fileName, shader_type type)
{
    if (handle_ <= 0)
    {
...
bool shader::CompileShaderFromString(const std::string & source, shader_type type)
{
    if (handle_ <= 0)
    {
        handle_ = glCreateProgram();
